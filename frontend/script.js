...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        ...
        const sourceItems = sources.map(source => {
            const displayText = `${source.course_title}${source.lesson_number !== null ? ' - Lesson ' + source.lesson_number : ''}`;

            if (source.lesson_link) {
                return `<li class="source-item"><a href="${source.lesson_link}" target="_blank" rel="noopener noreferrer" class="source-link">${displayText}</a></li>`;
            } else {
                return `<li class="source-item">${displayText}</li>`;
            }
        }).join('');
